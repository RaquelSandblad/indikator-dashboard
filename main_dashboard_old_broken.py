...
from scb_api_clean import SCBDataSource  # Ny ren SCB API-klass
from utils import (
...
def main():
    ...
    try:
        ...
        with st.sidebar:
            ...
            # Tillfälligt avaktiverad under SCB API-fix
            st.write("SCB API - Testar aktuell 2024 data...")
            scb_test = SCBDataSource()
            try:
                test_data = scb_test.fetch_population_data()
                if not test_data.empty:
                    latest_year = test_data["År"].max()
                    st.write(f"✅ SCB API - Senaste data: {latest_year}")
                else:
                    st.write("⚠️ SCB API - Ingen data")
            except Exception as e:
                st.write(f"❌ SCB API - Fel: {e}")
...
def show_home_page():
    ...
    # Hämta snabbstatistik med ny SCB API
    scb = SCBDataSource()
    try:
...
def show_population_page():
    ...
    try:
        ...
        scb = SCBDataSource()
        pop_data = scb.fetch_population_data()  # Kungsbacka kommunkod
...
def show_locality_page():
    ...
    if selected_locality:
        ...
        with col3:
            ...
            try:
                scb = SCBDataSource()
                pop_data = scb.fetch_population_data()  # Kungsbacka kod
                if not pop_data.empty:
                    latest_year = pop_data["År"].max()
...
def show_admin_page():
    """Sida för datakällor och API-status - Temporärt förenklad"""
    
    st.header("Datakällor & API:er")
    st.info("Admin-sidan är temporärt förenklad under uppdatering av SCB API.")
    ...
    with col1:
        try:
            # regions = scb.get_regions()  # Inte implementerad än
